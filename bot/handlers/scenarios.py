...
async def process_source_selection(callback: CallbackQuery, state: FSMContext, db_pool: asyncpg.Pool):
    ...
    action = parts[1]
    source_key = parts[2] if len(parts) > 2 else None
    ...
    elif action == "toggle" and source_key:
...
async def process_timezone_and_save(message: Message, state: FSMContext, db_pool: asyncpg.Pool, scheduler: AsyncIOScheduler):
    ...
    async with db_pool.acquire() as conn:
        try:
            ...
            await message.reply(get_text(lang_code, 'scenario_name_exists_error'))
            channel_id = data.get('channel_id')
            keyboard = await get_scenarios_menu_keyboard(channel_id, lang_code, db_pool)
            channel_info = await db_pool.fetchrow("SELECT channel_name FROM channels WHERE channel_id = $1", channel_id)
            if channel_info:
                await message.answer(get_text(lang_code, 'scenarios_menu_title', channel_name=channel_info['channel_name']), reply_markup=keyboard.as_markup())
...
async def _show_manage_scenario_menu(event: Message | CallbackQuery, db_pool: asyncpg.Pool, state: FSMContext, bot: Bot):
    ...
    if isinstance(event, CallbackQuery):
        scenario_id = int(event.data.split("_")[-1])
    ...
    if not scenario:
        if isinstance(event, Message):
            await event.answer("Ошибка: сценарий не найден.")
        return
...
async def edit_scenario_name_prompt(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(get_text(callback.from_user.language_code, 'enter_new_scenario_name'))
    await callback.answer()
...
async def edit_scenario_theme_prompt(callback: CallbackQuery, state: FSMContext):
    await state.set_state(ScenarioEditing.editing_theme)
    await callback.message.edit_text(get_text(callback.from_user.language_code, 'enter_new_scenario_theme'))
    await callback.answer()
